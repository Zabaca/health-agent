...
export default async function PdaReleaseViewPage({
  params,
}: {
  params: Promise<{ patientId: string; releaseId: string }>;
}) {
  ...
  const relationshipLabel = relation.relationship && relation.relationship !== 'Other'
    ? `Authorized Representative (${relation.relationship})`
  ...
  return (
    <Stack gap="xl" className="release-content">
      ...
      <Breadcrumbs mb={0} className="no-print">
        <Anchor component={Link} href={`/representing/${patientId}`} size="sm">{patientName}</Anchor>
        <Anchor component={Link} href={`/representing/${patientId}/releases`} size="sm">HIPAA Releases</Anchor>
        <Text size="sm">{release.releaseCode ?? releaseId}</Text>
      </Breadcrumbs>

      <Group justify="space-between" align="center" className="no-print">
        <Group gap="sm">
          <Button component={Link} href={`/representing/${patientId}/releases`} variant="subtle" leftSection={<IconArrowLeft size={16} />} px={0}>
            Back
          </Button>
          ...
          <Text size="sm" fs="italic">
            ...
            {release.releaseAuthAgent
              ? <>
                  ...
                  ({relationshipLabel}) as the acting agent in requesting medical records on the patient&apos;s behalf.
                </>
            ...
            <Paper withBorder p="sm" radius="md">
              <Stack gap="md">
                ...
                <SimpleGrid cols={3}>
                  ...
                  <Field label="Relationship to Patient" value={relationshipLabel} />
