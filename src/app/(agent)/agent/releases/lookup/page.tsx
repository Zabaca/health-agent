...
export default function AgentReleaseLookupPage() {
  ...
  const handleLookup = async () => {
    ...
    setSearched(false);
    try {
      const res = await apiClient.agent.releaseLookup({ params: { code: trimmed } });
      if (res.status === 200) {
        setResult(res.body);
      } else {
        setResult(null);
        ...
        result ? (
          <Paper withBorder p="md" radius="md" maw={500}>
            <Stack gap="xs">
              ...
              <Anchor
                component={Link}
                href={`/agent/patients/${result.patientId}/releases/${result.id}`}
                size="sm"
              >
                View Release →
              </Anchor>
