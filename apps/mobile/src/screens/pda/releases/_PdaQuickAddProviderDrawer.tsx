...
import { listRepresentingProviders, replaceRepresentingProviders, type MyProviderInput, type UserProvider } from "@/lib/api";
...
export function PdaQuickAddProviderDrawer({ patientId, visible, onClose, onSaved }: Props) {
  const t = useTheme();
  ...
  async function handleSave() {
    ...
    if (isInsurance && !membershipIdFront) {
      Alert.alert("Required", "Please add the insurance card front image.");
      return;
    }
    if (isInsurance && !membershipIdBack) {
      Alert.alert("Required", "Please add the insurance card back image.");
      return;
    }
    ...
    try {
      ...
      await replaceRepresentingProviders(patientId, [...current.map(toInput), input]);
      reset();
      onSaved();
    } catch (e) {
