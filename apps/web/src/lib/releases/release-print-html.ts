...
// Shared HIPAA-release print HTML builder. Used by both the patient-owned route
// (GET /api/releases/[id]/print-html) and the PDA-scoped route
// (GET /api/representing/[patientId]/releases/[releaseId]/print-html) so the two
// surfaces produce byte-identical documents.
...
function buildProviderSection(p: ProviderSection, index: number): string {
...
export function buildReleaseHtml(release: ReleaseHtmlInput): string {
  const { ssn, dateOfBirth, ...r } = release;
...
<body>
  ...
  <div class="page">
    ...
    <div class="section">
      ...
      ${r.providers.map((p, i) => buildProviderSection(p, i)).join("")}
    </div>
    <div class="footer">
